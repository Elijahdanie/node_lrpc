...
const generateClientCode = (controllerName, className, methodName, request, response, LRPC, isSocket) => {
    ...
    return `
        static async ${className}(data: ${getTypeDefinitions(request.name)} | ${request.name}${isSocket ? `, onMessage: (message: ${response.name}, socket: ISocketHandler) => void`:""}): ${isSocket ? `Promise<any> {` : `Promise<${response.name}> {` } 
...
const createServiceClient = (LRPC) => {
    ...
    LRPC.redis.set(`${LRPC.application}-${process.env.NODE_ENV}:${LRPC.service}-host`, process.env.SERVICEHOST);
    setTimeout(async () => {
...
const createFEClient = (LRPC) => {
    ...
    export const requestSocket = async (url: string, procedure: string, data: any, onMessage: (message: any, socket: ISocketHandler) => void) => {
        try {
